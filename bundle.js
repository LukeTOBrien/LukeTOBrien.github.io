...
module.exports = AFRAME.registerComponent('checkpoint-controls', {
  ...
  initAnimation: function initAnimation() {
    ...
    this.el.addEventListener('navigation-start', function (e) {
      if (_this.el.parentElement.lastElementChild.nodeName == "A-ANIMATION") {
        _this.el.parentElement.lastElementChild.remove();
        _this.el.parentElement.object3D.rotation.copy(new THREE.Vector3());
        _this.el.parentElement.rotationObject = {};
        // this.el.parentElement.object3D.quaternion.copy(new THREE.Quaternion());
      }
    });

    this.el.addEventListener('navigation-end', function (e) {
      if (e.detail.checkpoint.components.checkpoint.data.animationId) {
        var animationId = e.detail.checkpoint.components.checkpoint.data.animationId;
        var animation = document.getElementById(animationId);

        //this.el.parentElement.object3D.rotation.copy(
        //  animation.parentElement.object3D.rotation
        //);

        _this.el.parentElement.appendChild(animation.cloneNode(false));
      }
    });

    /*
    this.el.addEventListener('navigation-end', e => {
      //this.el.setAttribute('position', e.detail.checkpoint.parentNode.attributes[0].nodeValue);
      var animation = e.detail.checkpoint.parentNode.previousElementSibling;
      if (animation.nodeName == 'A-ANIMATION') {
        this.el.parentElement.appendChild(animation.cloneNode(false));
      }
      
    });
    this.el.addEventListener('navigation-start', e => {
      // Remove the animation
      let animation = this.el.parentElement.lastElementChild;
      if (animation.nodeName == 'A-ANIMATION') {
        animation.remove();
        
      }
      this.el.parentElement.object3D.rotation.copy(new THREE.Vector3());
      
          let animation = this.el.parentElement.lastElementChild;
    if (animation.nodeName == 'A-ANIMATION') {
      animation.remove();
      let realPos = this.el.object3D.getWorldPosition();
      //let offset = realPos.clone();
      //offset.sub(this.position);
      this.position.copy(realPos).applyMatrix4(this.el.object3D.matrix);
      this.targetPosition.applyMatrix4(this.el.object3D.matrix)
    }
...
module.exports = AFRAME.registerComponent('checkpoint', {
  schema: {
    ...
    animationId: { default: undefined, type: 'string' }
  },
